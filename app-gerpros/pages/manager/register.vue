    ...
    <div class="w-full max-w-md p-8 bg-white rounded-lg shadow-md">
      ...
      <div v-if="!auth.isLoggedIn" class="text-center">
        <p class="text-gray-600">
          You are not logged in. Please <nuxt-link to="/" class="text-blue-500 hover:underline">login</nuxt-link>.
        </p>
      </div>
      ...
      <p v-if="message" class="mt-4 text-sm text-red-500 text-center">{{ message }}</p>
    </div>
  </div>
</template>
...
const validatePassword = (password: string) => {
  ...
  if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) return '密碼需要包含至少一個特殊字符';
  return null;
};
